  ...
  useEffect(() => {
    if (userDetail != null) {
      if (
        ...
        userDetail?.email_verified_at == null ||
        userDetail?.email_verified_at == ''
      ) {
        //   Alert.alert("called")
        Toast.show({
    ...
    >
      <View style={styles.formContainer}>
        <Text style={styles.subTitle}>
          Hi {params?.data?.customer_details?.name},
        </Text>
        <Text style={[styles.subTitle, { marginTop: 5 }]}>
          We noticed that your email ({params?.data?.customer_details?.email})
          is not verified with Drift. Please take a moment to verify your
          account
        </Text>
        <Text style={[styles.subTitle, { marginTop: 10 }]}>
          Click the button below to receive an email with further instructions.
        </Text>
      </View>
      <DSButton
      ...
      <View style={styles.signUpContainer}>
        ...
        <TouchableOpacity onPress={() => resendEmail(params?.data?.token)}>
          <Text style={[styles.signUpText, { color: AppColors.THEME_GREEN }]}>
            Resend Email
          </Text>
        </TouchableOpacity>
      </View>
